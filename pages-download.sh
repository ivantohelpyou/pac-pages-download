...
# 13. Generalized the script by replacing specific environment names and table names with placeholders.
...
if pac auth list | grep -q "$ENVIRONMENT"; then
    echo "Already authenticated to the $ENVIRONMENT environment."
else
...
if [ -z "$1" ]; then
    ...
    if [[ $index =~ ^[0-9]+$ ]] && [ $index -ge 0 ] && [ $index -lt ${#powerpages_list[@]} ]; then
        ...
        webapi_table_enabled_id=$(generate_uuid)
        webapi_table_fields_id=$(generate_uuid)
...
adx_sitesetting:
  - adx_sitesettingid: $webapi_table_enabled_id
    adx_value: true
    adx_name: Webapi/${TABLE_NAME}/enabled
  - adx_sitesettingid: $webapi_table_fields_id
    adx_value: *
    adx_name: Webapi/${TABLE_NAME}/fields
EOL
